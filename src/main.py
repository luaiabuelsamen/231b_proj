...
def run_main(experimentalRun,visualize):
    ...
    print('   score =',np.abs(posErr_x[-1])+np.abs(posErr_y[-1])+np.abs(angErr[-1]))
    ...
    if not np.isnan(score):
        ...
        score = np.abs(posErr_x[-1])+np.abs(posErr_y[-1])+np.abs(angErr[-1])
